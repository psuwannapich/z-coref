...
class LeftOversCollator:
    ...
    def __call__(self, batch):
        ...
        if "gold_clusters" in batch:
            max_num_clusters, max_max_cluster_size = max(batch["num_clusters"]), max(
                batch["max_cluster_size"]
            )
            if max_num_clusters and max_max_cluster_size:
                padded_clusters = [
                    pad_clusters(cluster, max_num_clusters, max_max_cluster_size)
                    for cluster in batch["gold_clusters"]
                ]
...
class DynamicBatchSampler:
    def __init__(
        self, dataset, collator, max_tokens, max_segment_len, max_doc_len=None
    ):
    ...
    def __iter__(self):
        ...
        for example in self.dataset:
            if self.max_doc_len is not None and example["length"] > self.max_doc_len:
                logger.info(
                    f'Skipping doc with len {example["length"]}. max_doc_len is {self.max_doc_len}'
                )
                continue
            if not batch:
