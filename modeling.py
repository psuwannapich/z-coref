...
class CorefModel(ABC):
    def __init__(
        ...
        if collator_class == PadCollator:
            ...
            raise NotImplementedError(
                f"Class collator {type(collator_class)} is not supported! "
        ...
        self.model, loading_info = coref_class.from_pretrained(
            self.model_name_or_path, config=config, output_loading_info=True
        )
        self.model.to(self.device)

        for key, val in loading_info.items():
            logger.info(
                f'{key}: {list(set(val) - set(["longformer.embeddings.position_ids"]))}'
            )
        t_params, h_params = [p / 1000000 for p in self.model.num_parameters()]
        logger.info(
            f"Model Parameters: {t_params + h_params:.1f}M, "
    ...
    def _create_dataset(self, texts, is_split_into_words):
        ...
        dataset = dataset.map(
            ...
            fn_kwargs={
                "tokenizer": self.tokenizer,
                "nlp": self.nlp if not is_split_into_words else None,
            },
        )
    ...
    def predict(
        ...
        ],  # similar to huggingface tokenizer inputs
